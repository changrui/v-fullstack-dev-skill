...
pub struct UpdateItemRequest {
    name  string [json: 'name' optional]     // optional: 字段可为空
...
// 实际项目中应替换为 sqlite_store.v 中的具体实现
interface ItemStore {
    list(limit int, offset int) ([]Item, int)  // 返回列表和总数
    get(id string) ?Item                      // 获取单个 item
    create(item Item) !int                    // 创建，返回新 ID 索引
    update(id string, item Item) !int         // 更新，受影响的行数
    delete(id string) !int                    // 删除，受影响的行数
}
...
pub fn (mut app App) update_item(mut ctx veb.Context) veb.Result {
    ...
    return ctx.json(json2.encode[ItemResponse](ItemResponse{
        items: [Item{id: id, name: req.name, value: req.value, updated: time.now().unix_milli()}],
        total: 1,
        page: 1,
        limit: 1,
        has_prev: false,
        has_next: false,
    }, json2.EncoderOptions{})), status: 200)
}
...
pub fn (mut app App) batch_create(mut ctx veb.Context) veb.Result {
    ...
    // 批量验证
    for item in items {
        if err := validate_create_item(item); err != none {
